...
%% API
-export([start_link/0,request_token/1,post_tweet/1,subscribe_to_term/1]).
...
subscribe_to_term(Term) ->
  gen_server:call(?MODULE, {call_subscribe_to_term, Term}, 10000).

post_tweet(Text) ->
  gen_server:call(?MODULE, {call_post_tweet, Text}, 10000).

request_token(Callback) ->
  gen_server:call(?MODULE, {call_request_token, Callback}, 10000).

get_token(Token, Secret, Pin) ->
  gen_server:call(?MODULE, {call_get_token, Token, Secret, Pin}).
%%--------------------------------------------------------------------
%% @doc
%% Starts the server
%%
...
handle_call({call_post_tweet, Text}, _From, State) ->
%%   Url = "https://api.twitter.com/1.1/statuses/update.json",
%%   Params = "status=" ++ Text,
%%   {ok, {Oauth_hstring}} = build_oath_call(Url, Params),
%%   io:format("Oauth_hstring = ~s~n", [Oauth_hstring]),
%%   {ok, {{Version, Code, ReasonPhrase}, Headers, Body}} = httpc:request(post, {Url, [{"Authorization", Oauth_hstring},
%%     {"Accept", "*/*"}, {"User-Agent", "Doms123"},
%%     {"Content-Type", "text/html; charset=utf-8"}],
%%     "application/x-www-form-urlencoded", Params},
%%     [{autoredirect, false}, {relaxed, true}], []),
%%   case Code of
%%     200 ->
%%       io:format("messsage accepted by cbsa Body is ~s~n", [Body]);
%%     _ ->
%%       io:format("there was a problem code was ~s~n", [Body])
%%   end,
  {reply, ok, State}.
